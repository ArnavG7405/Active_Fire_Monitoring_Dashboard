...
def expand_industrial_database():
    print("Step 1: Querying Overpass API for new heavy industries in India...")
    
    # ---------------------------------------------------------
    # MANDATORY HEADERS TO BYPASS 406 ERROR
    # ---------------------------------------------------------
    headers = {
        "User-Agent": "Geospatial-Fire-Pipeline/1.0",
        "Accept": "*/*"
    }
    ...
    if not elements:
        print("No new elements found. Exiting.")
        return
    ...
    with engine.begin() as conn:
        ...
        for el in elements:
            ...
            # ---------------------------------------------------------
            # STRICT POSTGIS SPATIAL DEDUPLICATION
            # ---------------------------------------------------------
            insert_query = text("""
